...
try:
    ...
    from models.Together_API import generate_with_Together_model
except:
    pass
...
class IO_System:
    ...
    def generate(self, model_input, max_tokens: int, num_return: int, stop_tokens):
        if isinstance(model_input, str):
            if self.api == "vllm":
                ...
                #     enable_chat_template=self.enable_chat_template
                # )
                gpt_response = generate_with_Together_model(
                    prompt=model_input,
                    n=num_return,
    ...
    def generate_deterministic(self, model_input, max_tokens: int, num_return: int, stop_tokens):
        if isinstance(model_input, str):
            if self.api == "vllm":
                vllm_response = generate_with_vLLM_model(
                    self.model,
