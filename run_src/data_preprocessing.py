...
def convert_options_to_dictionary(options_list):
    """
    Convert a list of options dictionaries to a dictionary with letter keys.
    Identifies the gold answer as the option with the highest percentage.

    Args:
        options_list (list): List of option dictionaries

    Returns:
        tuple: (options_dict, gold_answer) where:
            - options_dict: Dictionary with keys A-G and values as option content
            - gold_answer: The letter of the option with highest percentage
    """
    options_dict = {}
    max_percent = -1
    gold_answer = None
    ...
    for option_data in options_list:
        # Extract the letter from the option string (first character)
        letter = option_data['option'].split('\n')[0]

        # Extract the content (everything after the letter and newline)
        content = option_data['option'].split('\n')[1] if '\n' in option_data['option'] else ''

        # Add to dictionary
        options_dict[letter] = content

        # Check if this is the highest percentage
        percent_value = int(option_data['percent'].strip('%'))
        if percent_value > max_percent:
            max_percent = percent_value
            gold_answer = letter

    return options_dict, gold_answer


def ambossqa():
    root = "data/amboss_tag"
    # level_directories = ["d1", "d2_1", "d3_1", "d4", "d5"]
    data = []
    level_directories = ["d5"]

    for directory in level_directories:
        for idx in range(1, 101):
            path = root + "/" + directory + "/"+ str(idx) + ".json"
            info = load_json(path)
            options_list = info['options']
            question = info['question']
            option, answer = convert_options_to_dictionary(options_list)
            item = {
                "id": idx,
                "problem": question,
            ...
            data.append(item)
    print(len(data))
    write_json(data, "data/MedQA/amboss_d5.json")
